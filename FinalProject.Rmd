...
```{r create-severity-score}
# Create a severity score for easier analysis
crash_severity <- crash_data %>%
  mutate(
...
factor_summary <- data.frame(
  ...
  Average_Severity = c(
    mean(time_severity$avg_severity, na.rm = TRUE),
    mean(weather_severity$avg_severity, na.rm = TRUE),
    mean(control_severity$avg_severity, na.rm = TRUE),
    mean(distraction_severity$avg_severity, na.rm = TRUE),
    mean(movement_severity$avg_severity, na.rm = TRUE)
  ),
...
factor_summary <- factor_summary %>%
  mutate(
    Average_Severity = round(Average_Severity, 3),
    Average_Severity = ifelse(is.nan(Average_Severity), NA, Average_Severity)
  )

# Print the summary table with better formatting
